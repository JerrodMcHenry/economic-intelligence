...
# change this pipeline ever computes an analytical consequence for
# (see app.domain.release_processing.SERIES_TO_COMPONENTS). Loaded for
# before/after evidence regardless of which of them this particular
# release's own mappings touch, because a change to one (e.g. Core PCE)
# can affect a section (e.g. Confirmation) that also depends on another
# (Core CPI) this release never mapped at all.
...
class ReleaseProcessingService:
    ...
    def _apply_changes_and_compute_analysis(
        ...
        comparison primitives -- exactly once per affected (component,
        period) pair, never once per individual observation row (see
        `app.domain.release_processing.affected_evaluation_periods`'s
        own docstring for why a batch of several changed dates that
        share one affected period must only be evaluated once)."""
        ...
        if not affected_pairs:
            ...
            # deterministic Inflation consumer (app.domain.release_processing
            # .components_for_series returned empty) -- write the data,
            # compute no analysis (there is none to compute).
            for record in observation_changes:
        ...
        before_observations = _load_canonical_observations(repo)
        ...
        after_observations = _load_canonical_observations(repo)
