...
class ReplayService:
    ...
    def replay_recorded_result(self, session: Session, recorded_monitor_result_id: int) -> ReplayResult:
        ...
        replayed_state = self._recompute(recorded, as_of)
    ...
    @staticmethod
    def _recompute(recorded: RecordedMonitorResult, as_of: dict[str, list[AsOfObservation]]) -> str | None:
        """Re-run the SAME deterministic primitive that produced the
        recorded row. No new economic logic lives here."""
        period = recorded.evaluation_period

        if recorded.monitor == "inflation":
            return compute_series_momentum_at(
                _as_observations(as_of[PRIMARY_SERIES_ID]), PRIMARY_SERIES_ID, period
            ).state
