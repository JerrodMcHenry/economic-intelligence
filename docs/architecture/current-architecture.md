...
| Rates Intelligence UI (Increment #30) | `frontend/src/pages/Rates.tsx`, `frontend/src/components/rates/`, `frontend/src/api/rates{,.types}.ts`, `frontend/src/lib/ratesFormat.ts` | The presentation surface for `rates_v1.0`, at `/rates`. Renders backend-computed values only: no spread, basis-point conversion, compensation, or percentile is ever calculated client-side, enforced by `frontend/src/test/no-rates-calculation.test.ts` plus the retargeted frontend guard in `tests/test_rates_architecture.py`. The yield curve is hand-drawn inline SVG (no charting dependency) with a real `<table>` alternative beneath it. Source and derived values render through structurally different provenance panels, so a calculated value can never look like one Treasury published. |
| Rates Monitor route (Increment #29) | `app/api/rates.py` | HTTP layer for `GET /api/v1/monitors/rates` (read-only canonical `rates_v1.0` result) and `POST /api/v1/rates/sync` (explicit, operator-invoked Treasury ingestion). Two routers in one module: the monitor route joins the shared `/monitors` prefix alongside Inflation/Labor, the sync route sits under `/rates` mirroring `POST /api/v1/releases/sync`. The GET route holds no Treasury client at all — a read is structurally incapable of triggering ingestion (checked by `tests/test_rates_architecture.py`). No FRED, no AI, no API key. |
...
| Treasury client (Increment #29) | `app/clients/treasury.py` (`TreasuryClient`) | Owns all U.S. Treasury XML communication, mirroring `FREDClient`'s boundary. Security-relevant by construction: the host and a closed two-dataset allow-list are module constants, an unknown dataset is refused before a request is built, year/month are range-checked and integer-formatted, redirects are disabled, and responses are capped at 1MB. No API key exists for these feeds, so none is configured or logged. |
...
| Rates Monitor service (Increment #29) | `app/services/rates.py` (`RatesMonitorService`) | Loads the six canonical Treasury series via `RatesRepository` and delegates every calculation to `app.domain.rates`. Database-only, read-only. Assembles levels, four session-counted change windows, historical percentile context, derived curve spreads and derived inflation compensation — each component independently reporting its own availability and, when unavailable, which of three alignment/data conditions caused it. |
| Rates ingestion service (Increment #29) | `app/services/rates_ingestion.py` (`RatesIngestionService`) | The write counterpart, a separate class rather than a mode — the same `ReleaseReadService`/`ReleaseSyncService` separation. Requires a `TreasuryClient`; fetches each dataset independently so one dataset failing never discards the other's ingested observations (`PARTIAL_FAILURE`), and records one `RatesIngestionRun` per attempt with counts and an exception class name only. |
| Rates repository (Increment #29) | `app/repositories/rates_repository.py` (`RatesRepository`) | All Rates SQL. Reuses `economic_series`/`economic_observations` unchanged (a par yield is a dated observation of a named series). `upsert_observation` returns `INSERTED`/`REVISED`/`UNCHANGED`, so idempotency and genuine-revision detection live here rather than in caller discipline; provenance rows advance `revision_count`/`last_revised_at` only on a real value change. |
...
| Series repository | `app/repositories/series_repository.py` (`SeriesRepository`) | All SQL for series/observations: upserts series metadata and observations within a caller-owned transaction; series lookup; local metadata search (`search_series`); filtered/ordered/paginated observation queries; unpaginated range queries and preceding-context queries. Reused as-is by every consumer added since Increment 005 — no analysis-, pipeline-, or discovery-specific repository methods were ever needed beyond `search_series` itself. `_upsert_observations` backs the plain `/series/{id}/sync`; release processing owns a separate write path (next-but-one row), never retrofitted here. As of Increment #31 it no longer overwrites blindly — it delegates to `ObservationVersionWriter` (below), which performs the canonical write and records a system-time version together, so a revision through this path no longer erases its predecessor. |
| Observation versioning (Increment #31) | `app/repositories/observation_versions.py` (`ObservationVersionWriter`, `ObservationVersionRepository`) | **The one and only place an observation value is written and versioned.** `ObservationVersionWriter.apply()` performs the canonical `economic_observations` write *and* records the append-only `observation_versions` row together, returning `INSERTED`/`REVISED`/`UNCHANGED` — decided by the value, never by the caller, so re-writing an identical value produces no new version and routine re-fetches cannot bury genuine revisions. All three write paths delegate to it, tagged `SERIES_SYNC` / `RELEASE_PROCESSING` / `RATES_INGESTION`; three separate implementations of half-open interval closing would drift, and a versioning layer correct on only some paths is worse than none because its gaps are invisible at read time. `ObservationVersionRepository.get_observations_as_of(series_id, at)` returns the series exactly as MacroChipz held it at `at` and **never falls back to the current value** — a missing version returns nothing, because a silent fallback would make every replay succeed. See [ADR-030](../adr/030-observation-system-time-versioning-and-deterministic-replay.md). |
| Deterministic replay (Increment #31) | `app/services/replay.py` (`ReplayService`), `app/models/replay.py` | Re-executes a frozen methodology against as-of inputs and compares the result to what was recorded at the time — `data known THEN + methodology used THEN + deterministic code = the result recorded THEN?`. It **re-runs rather than reads back**: the `RecordedMonitorResult` is loaded only to compare against, and the same `app.domain` primitives release processing itself used are called directly, so replay depends on the methodology rather than on another service's internals. Refuses rather than guesses: insufficient version coverage or an unrecognized `methodology_id` yields `NOT_REPLAYABLE` with an explicit reason, checked *before* any calculation — feeding an empty input set to the methodology would return `INSUFFICIENT_DATA`, which looks like an economic finding but is really a storage gap. Read-only; never updates the recorded row it is checking, so `recorded_monitor_results` stays append-only (ADR-025). No HTTP route and no UI. |
| Point-in-time intelligence history (Increment #32) | `app/models/monitor_history.py`, `app/repositories/monitor_history_repository.py`, `app/services/monitor_history.py`, `app/api/monitor_history.py` | The read-only product surface over #31's temporal foundation: `GET /api/v1/monitors/{monitor}/history` and `.../history/{recorded_result_id}`. `{monitor}` is a two-value Literal, so `/monitors/rates/history` is a 422 — Rates has observation versioning but records no monitor state, and an empty list would imply a history that does not exist. **`historical_inputs` is read off the methodology's own evidence** (`InflationMetricEvidence.endpoint_date_*`, `LaborObservationEvidence.observation_date`), never from a lookback window this layer invents, so the input set cannot drift from what the calculation actually used. The then-vs-today comparison calls the SAME `_at` primitive twice — once over as-of data, once over current observations, at the same evaluation period — and compares **evidence against evidence** so the two sides always speak the methodology's own units (`labor_v1.0` reports PAYEMS in jobs; storage holds FRED-native thousands). Refuses rather than fakes: across methodology versions no `current_state` is produced at all, and `state_differs` stays `null` rather than defaulting to `false`. `related_changes` are narrowed to observations this result used and the remainder is counted, never dropped — the shared `release_check_run_id` is a real link but records no causation (ADR-023). |
| MacroChipz Analyst — deterministic context (Increment #33) | `app/models/analyst.py`, `app/services/analyst_context.py` | Assembles a typed, versioned `analyst_context_v1` packet from canonical services (Inflation, Labor, Rates, Monitor History — all read-only) and assigns every citable item a stable evidence id. Imports no provider: context assembly would behave identically if no language model existed. Values are pre-formatted strings carrying their units (`"158,268,000 jobs"`), because a bare number whose unit must be inferred is the class of mistake that produced a 1,000x error inside #32's own deterministic code. Rates carries `canonical_state: null` — `rates_v1.0` publishes levels and derived metrics, not a classification, and fabricating a state to make packets uniform would invent intelligence the engine does not produce. |
| MacroChipz Analyst — model boundary (Increment #33) | `app/services/analyst.py`, `app/services/analyst_prompt.py`, `app/api/analyst.py` | **The only module in the application that sends context to a language model, and structurally incapable of reaching data**: it imports no SQLAlchemy, no `app.db`, no repository and no canonical service, and no method accepts a `Session` — the route builds the packet, closes the session, then calls it with the packet alone (ADR-032, asserted by AST in `tests/test_analyst_architecture.py`). **No tools and exactly one `responses.create` per request**, the direct consequence of ADR-018's measured ~60% schema-violation rate under model-driven orchestration. Output is schema-validated and every evidence reference is checked against the packet actually sent — an id MacroChipz did not publish is dropped and counted, and a valid id is re-resolved from the packet so even a correct citation cannot carry model-authored provenance. `GET /api/v1/analyst/availability` answers `200 {available:false}` when unconfigured, naming no setting; `POST /api/v1/analyst/explain` takes only a context REFERENCE plus a question, with `extra="forbid"`, so a browser can never supply canonical state. Every provider failure maps to a contained 503 and none can alter canonical intelligence. |
...
| Rates engine (Increment #29) | `app/domain/rates.py` (`to_basis_points`, `basis_point_change`, `change_over_sessions`, `historical_session_changes`, `rank_against`, `align_on_exact_date`, `spread_series`, `inflation_compensation_series`) | Pure, deterministic implementation of `rates_v1.0` (`docs/methodology/rates-v1.0.md`). Three properties it enforces literally: windows are counted in **published observations, never calendar days** (no weekend/holiday fallback policy to get wrong); two-series metrics require an **exact shared date** (no nearest-date match, no forward-fill); and missing is never zero — every function distinguishes "no data" from "no change" in its return type. Assigns no state label to any rate: V1 reports measurements and percentiles rather than invented thresholds. |
...
| Rates response models (Increment #29) | `app/models/rates.py` | The one canonical definition of `rates_v1.0`'s constants, canonical series IDs, provider identity, attribution string, and session-counted change windows. Carries two structurally distinct provenance types: `SourceProvenance` (provider/dataset/upstream field/URL/retrieval time/revision count) for a sourced observation, and `DerivedProvenance` (methodology ID, calculation, inputs, calculation time — deliberately with no provider field) for a computed one, so a derived metric can never masquerade as a directly sourced observation. |
...
**Increment #29 additions (Rates).** Two additive tables; no existing table, column, or row was changed.

- `observation_provenance` — one row per `(economic_series_id, observation_date)`: provider, dataset, upstream field name, source URL, `retrieved_at`, `first_seen_at`, `revision_count`, `last_revised_at`. Deliberately a separate table rather than columns on `economic_observations`, because provenance describes the *retrieval event* rather than the economic fact, and every pre-#29 observation legitimately has none — a missing row means "not recorded", never a silently assumed source.
- `rates_ingestion_runs` — one row per ingestion attempt: provider, datasets requested, status (`SUCCEEDED`/`PARTIAL_FAILURE`/`FAILED`), start/end, duration, observations received/inserted/revised, failed datasets, and an exception class name. Mirrors `release_check_runs`' role for release processing: it proves an ingestion was *attempted* and how it ended, independent of whether anything changed. Never stores an upstream payload.

Migration `b7c41d92e8a3` (reversible; verified upgrade → downgrade → upgrade against the isolated drift database).

**Increment #31 additions (observation versioning).** One additive table; no existing table, column, or row was changed, and no existing reader was modified.

- `observation_versions` — append-only system-time history for every observation value: `(economic_series_id, observation_date, value, recorded_from, recorded_to, change_type, origin, is_backfilled)`. Each row is a half-open interval `[recorded_from, recorded_to)` answering "when did MacroChipz hold this value?" — deliberately **system time only**, with no valid-time axis, because providers do not reliably supply publication timestamps and a half-honest column is worse than an absent one ([ADR-030](../adr/030-observation-system-time-versioning-and-deterministic-replay.md)). `economic_observations` remains exactly what it was: the current-value cache. Invariants are enforced by PostgreSQL, not application discipline — a partial unique index permits at most one open version per `(series, observation_date)`, a unique constraint forbids two versions starting at the same instant, and a check constraint rejects `recorded_to <= recorded_from` (a zero-length interval would be invisible to every as-of query). Rows created by the migration's own backfill carry `is_backfilled = true` and `change_type`/`origin` = `BACKFILL`, with `recorded_from` set to the observation's `created_at` — honest support for "this value existed by then" and nothing stronger; pre-#31 revisions are permanently unrecoverable.

Migration `c3f8a1d47b62` (reversible; verified upgrade → downgrade → upgrade against the isolated drift database, with downgrade preserving every canonical observation).
...
### Intelligence History (Increment #32)

`components/history/` (`IntelligenceHistorySection`, `HistoricalResultDetail`, `ReplayStatusBadge`), `api/monitorHistory.ts` + `.types.ts`, `api/useMonitorHistoryDetail.ts`, `lib/historyCopy.ts`, `lib/historyLabels.ts`.

One section on `/inflation` and `/labor`, placed immediately before each page's methodology disclosure so the page reads current → recent → historical → how it works. On Labor this extends `labor-ui-v1.md` §7's frozen hierarchy to eight sections at position 7 specifically to preserve that contract's own deliberate adjacencies (“Latest Data Detected” stays next to “Relevant Release” because they concern the same release).

Four properties are load-bearing and each is guarded by `src/test/no-history-derivation.test.ts`:

- **No economic derivation.** `comparison`, `state_differs`, `changed_input_count` and `status` are rendered verbatim. The guard forbids subtracting then/today values, comparing them to decide whether something was revised, counting changed inputs by hand, and assigning a replay verdict literal — the same per-line, string-literal-stripped scan `no-rates-calculation.test.ts` established.
- **Replay status uses the `feedback-*` family, economic states use `state-*`.** A replay outcome is a verdict about MacroChipz's own integrity, not an economic classification, so the two token families stay disjoint and each is used for what it is for. A `MISMATCH` keeps the error treatment and a screen-reader description naming it a data-integrity issue.
- **Then and today never collide.** Two permanently distinct headings, asserted to differ, with the "today" heading forbidden from containing a past-tense knowing verb.
- **No database vocabulary in the primary experience.** "System-time", "recorded_to", "temporal", "half-open" and "bitemporal" are asserted absent from every user-facing string.

`useMonitorHistoryDetail` is a specialized sibling of `useApiResource` (the `useSinceLastVisit` precedent) for two reasons `useApiResource` cannot serve: it is parameterized by a recorded-result id, and it must not fetch until a row is opened — so detail is loaded on demand, not a dozen requests per page load. Its "loading" is derived rather than stored. This is also the one place the page does not own the resource, because there is one per expandable row rather than a single owner.

### Ask MacroChipz (Increment #33)

`components/analyst/AskMacroChipz.tsx`, `api/analyst.ts` + `.types.ts`, `lib/analystCopy.ts`, plus `apiPost` in `api/client.ts` (the first and only POST this frontend makes).

One compact contextual surface per analytical page — Inflation, Labor, Rates, and inside #32's already-disclosed history detail. Not a chat: no thread, no memory, no persona; a question and an answer about the page in view. Absent from Home and Overview, where a standing guard forbids AI imports entirely.

Four properties, each guarded by `src/test/no-analyst-derivation.test.ts`:

- **No economic derivation.** The component collects a question, names the page's context, calls the API and renders the validated response. It never scores an answer, filters evidence, decides which citation is real, or computes a value — the extension of ADR-021's rule from curated explanations to generated ones.
- **The browser supplies no truth.** The request type carries a context type, an optional recorded-result id and a monitor. The guard asserts it contains no state, evidence, value or methodology field, and that the client posts only to the Analyst's own endpoint.
- **Suggested questions are deterministic UI copy**, written and reviewed in `analystCopy.ts`. Generating them would put an unreviewed prompt in the user's mouth and make every page load billable.
- **The Analyst is optional at the UI layer.** Availability is fetched as its own independent resource and passed in; unavailable renders one sentence and nothing else on the page changes. User-facing copy names no provider, model or setting.
