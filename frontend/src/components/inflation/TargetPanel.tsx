...
export function TargetPanel({ target }: { target: TargetResult }) {
  return (
    <section aria-labelledby="target-heading">
      <h2 id="target-heading" className="text-sm font-medium text-neutral-500">
        Target / level
      </h2>
      {!target.available ? (
        <p className="mt-2 text-sm text-neutral-400">Target data unavailable.</p>
      ) : (
        <div className="mt-3 flex flex-wrap items-baseline gap-x-10 gap-y-2">
          <div>
            <p className="text-xs font-medium uppercase tracking-wide text-neutral-400">Headline PCE</p>
      ...
      <p className="mt-2 text-xs text-neutral-500">{formatPeriod(target.calculation_period)}</p>
      <div className="mt-3">
