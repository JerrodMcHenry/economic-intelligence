  ...
  return (
    <div className="rounded-lg border border-line bg-surface p-4">
      ...
      <div className="mt-2 flex flex-wrap items-baseline gap-x-3 gap-y-1">
        ...
        <span className="text-xl font-semibold tabular-nums text-fg">{formatPercent(series.r_12m)}</span>
        <span className="text-xs text-fg-muted">12M</span>
        <ExplanationTrigger explanation={explanation} />
      </div>
      <p className="mt-1 text-xs text-fg-muted">{formatPeriod(series.calculation_period)}</p>
      <div className="mt-2">
  ...
  return (
    <section aria-labelledby="headline-context-heading">
      <div className="flex items-center gap-1.5">
        <h2 id="headline-context-heading" className="text-sm font-medium text-fg-muted">
          Headline context
        </h2>
